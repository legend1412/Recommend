...
class Model:
    ...
    def load_data(self):
        if os.path.exists('data/train.txt'):
            ...
            fw.write('label,price,pub_month,click,score,judge,rec_most,rec_more,rec_normal,rec_bad,'
                     'rec_morebad,readed,reading,readup\n')
            fw.write('\n'.join(result))
            fw.close()
    ...
    def train_model(self):
        label = 'label'
        x_columns = [x for x in self.train_data.columns if x not in [label]]
    ...
    def evalute_model(self):
        label = 'label'
        x_columns = [x for x in self.test_data.columns if x not in [label]]
        x_test = self.test_data[x_columns]
        y_test = self.test_data[label]
        y_pred = self.gbdt.predict_proba(x_test)
        new_y_pred = list()
        for y in y_pred:
            # y[0]表示label=0的概率，y[1]表示样本label=1的概率
