...
def login(request):
    if request.method == 'GET':
        ...
        return JsonResponse(
            {'code': 1, 'data': {'username': request.POST.get('username'), 'tags': request.POST.get('tags')}})
...
def home(request):
    ...
    if tag == 'all':
        ...
        # 如果用户选择标签，执行下面的逻辑
            ...
            # 用户选择的标签
            choose_tags = request.session['tags'].split(',') if 'tags' in request.session.keys() else list()
            # 用户有点击行为的标签
            click_tags = [one['tag'] for one in History.objects.filter(name=uname).values('tag').distinct()]
            # 拼接标签
            choose_tags.extend(click_tags)
            ...
            for book in all_books:
                features = [book.price, book.publish_month, book.click, book.score, book.judge, book.rec_most,
                            book.rec_more, book.rec_normal, book.rec_bad, book.rec_morebad, book.readed, book.reading
                    , book.readup]
    ...
    else:  # 表示用户查过的是具体标签下的图书数据
        books = Book.objects.filter(tag=tag).order_by('-score')
        total = books.__len__()
...
def one(request):
    ...
    write_to_mysql(name=request.GET.get('username'), time=format_local_time(), action='查看', object=one.name,
                   tag=one.tag, )
    return JsonResponse({
        'code': 1,
        'data': {
