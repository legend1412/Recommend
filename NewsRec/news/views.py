...
from news.models import News, Cate, Newsim, Newbrowse
import time
...
def one(request):
    ...
    newone = News.objects.filter(new_id=newid)[0]
    # 将用户的点击新闻信息写入数据库
    ...
    if newsim_list.__len__() == 0:
        newsim_list = News.objects.filter(new_cate_id=newone.new_cate).order_by('-new_seenum')[:5]
        flag = 'hot'
    # 拼接相似新闻信息
    ...
    for sim_one in newsim_list:
        ...
        cor = 0.0
        if flag == 'sim':
            sim_one_mess = News.objects.filter(new_id=sim_one.new_id_sim)[0]
