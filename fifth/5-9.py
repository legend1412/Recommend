...
class LFM:
    ...
    初始化参数
        randn:从标准正态分布中返回n个值
        pd.DataFrame:columns指定列顺序，index指定索引
    """
    ...
    """
    使用随即梯度下降算法求解参数，同时使用L2正则化防止过拟合
    eg:E=1/2*(y-predict)^2,predict=matrix_p*matrix_q
    ...
    """
    训练模型，每次迭代都要降低学习率，刚开始由于离最优值较远，因此下降较快，当到达一定程度后，就要减小学习率
    """
        ...
        for step in range(0, self.iter_count):
            ...
            for user_id, item_dict in self.items_dict.items():
                ...
                for item_id in item_ids:
                    e1 = self._loss(user_id, item_id, item_dict[item_id], step)
                    self._optimize(user_id, item_id, e1)
            self.lr *= 0.9
        self.save()
    ...
    # 模型效果评估，从所有user中随机选取10个用户进行评估，评估方法未：绝对误差（AE）
    def evaluate(self):
        self.load()
