...
class GetSongMess:
    ...
    def get_song_mess(self):
        ...
        for id in self.song_ids:
            try:
                ...
                try:
                    ...
                    lysic = 'null'
                self.write_to_file(self.ids_all_file + 'songs_mess_all.txt',
                                   ' |+| '.join(url_1_list + url_2_list + url_3_list))
                self.write_to_file(self.ids_all_file + 'songs_lysics_all.txt', str(res_1_json['id']) + '\t' + lysic)
                i += 1
            except Exception as e:
                print('error:%s' % e)
        ...
        if self.error_ids.__len__() != 0:
            ...
            self.write_to_file(self.error_ids_file, ','.join(self.error_ids))
        print('歌曲信息获取完成。。。')
    ...
    def filter_id(self):
        ...
        for one in self.song_ids:
            if one not in ids_list_not:
                print(i)
                i += 1
                self.write_to_file(self.ids_all_file + 'not_get_ids_%s.txt' % str(int(i / 50000)), one)
