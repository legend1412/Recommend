...
class RecPlayLit:
    ...
    def load_data(self):
        ...
        print('标签信息|用户打标统计|歌单对应标签信息统计 完成！')
    ...
    def create_user_feature_prefer_matrix(self):
        ...
        for user_id in self.user_tags_count_dict.keys():
            ...
            for tag in self.user_tags_count_dict[user_id].keys():
                ...
                # 部分用户对所使用的标签都是1次，self.user_tags_count_dict[user_id][tag]-u_avg+1
    ...
    def write_to_file(self):
        ...
        for user_id in self.user_playlist_prefer_dict.keys():
            sorted_user_prefer = sorted(self.user_playlist_prefer_dict[user_id].items(), key=lambda one: one[1],
                                        reverse=True)
...
if __name__ == '__main__':
    rec_playlist = RecPlayLit()
    rec_playlist.write_to_file()
