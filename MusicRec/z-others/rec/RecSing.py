...
class RecSing:
    ...
    def item_similarity_best(self):
        ...
        if os.path.exists('data/singer_sim_singer.json'):
            item_sim = json.load(open('data/singer_sim_singer.json', 'r', encoding='utf-8'))
            print('歌手相似从文件加载！')
            return item_sim
        item_user_count = dict()  # 得到每个物品有多个用户产生过行为
        count = dict()  # 共现矩阵
        for user, item in self.user_singer_dict.items():
            for i in item.keys():
                ...
                for j in item.keys():
                    ...
                    if self.user_singer_dict[user][i] > 0.0 and self.user_singer_dict[user][i] > 0.0 and i != j:
                        count[i][j] += 1
    ...
    def write_to_file(self):
        ...
        for user_id in self.user_singer_score_dict.keys():
            sort_user_singer_prefer = sorted(self.user_singer_score_dict[user_id].items(), key=lambda one: one[1],
                                             reverse=True)
