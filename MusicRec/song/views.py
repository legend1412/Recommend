...
def get_song_all(request):
    ...
    # 全部歌曲
    if tag == "all":
        song_tags_list = Song.objects.all().values("song_id", "song_name", "song_publish_time").order_by(
            "-song_publish_time")
        # 拼接歌曲信息
...
def get_song_one(request):
    ...
    writebrowse(user_name=request.GET.get("username"), click_id=song_id, click_cate="3",
                user_click_time=formatlocaltime(), desc="查看歌曲")
    return JsonResponse({
        "code": 1,
        "data": [
            {
                ...
                "song_rec": get_rec_based_one(song_id)
            }
        ]
    })
