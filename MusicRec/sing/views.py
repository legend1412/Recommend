...
from sing.models import Sing, SingSim, SingTag
from user.views import writebrowse, formatlocaltime
from song.models import Song


def get_sing_all(request):
    # 接口传入的tag参数
    ...
    return {'code': 1, 'data': {'total': total, 'sings': _list, 'tags': get_all_sing_tags()}}


# 获取所有歌手标签
def get_all_sing_tags():
    tags = set()
...
def get_sing_one(request):
    sing_id = request.GET.get('id')
    writebrowse(user_name=request.GET.get('username'), click_id=sing_id, click_cate='4',
                user_click_time=formatlocaltime(), desc='查看歌手')
        ...
        "data": [
            {
                ...
                "sing_url": one.sing_url,
                "sing_rec": get_rec_based_one(sing_id),
                "sing_songs": get_singer_song(sing_id)
            }
        ]
    })
