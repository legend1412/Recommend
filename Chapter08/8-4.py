...
class ChurnPredWithGBDTAndLR:
    ...
    def train_model(self):
        ...
        x_clolumns = [x for x in self.train.columns if x not in [lable, customer_id]]
        x_train = self.train[x_clolumns]
        ...
        # 模型融合
        gbdt_lr = LogisticRegression(max_iter=1000)
        enc = OneHotEncoder()
    ...
    def evaluate(self, enc, gbdt, gbdt_lr):
        ...
        print('GBDT-LR-MSE:%.4f' % mean_squared_error(y_test, new_gbdt_lr_y_pred))
        print('GBDT-LR-Accuracy:%.4g' % metrics.accuracy_score(y_test.values, new_gbdt_lr_y_pred))
        print('GBDT-LR-AUC Score:%.4g' % metrics.roc_auc_score(y_test.values, new_gbdt_lr_y_pred))


if __name__ == '__main__':
    pred = ChurnPredWithGBDTAndLR()
    enc_result, gbdt_result, gbdt_lr_result = pred.train_model()
    pred.evaluate(enc_result, gbdt_result, gbdt_lr_result)
